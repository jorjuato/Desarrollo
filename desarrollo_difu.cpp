...
///	Autor: Jorge Ibáñez, con la inestimable colaboración de Alberto Pascual y Alvaro Muñoz
/// 
///________________________________________________________________________________________________________
///
///========================================================================================================
...
int inicializa(void)
{
	...
	{	for (j=0; j<ANCHO; j++, k++) 
		{ //	Llamada a crear_celula recursiva hasta completar la poblacion inicial
			...
			for (s=1; s<MAXGEN_SEN-1; s++)  
				espDifusivo2D[i][j][s] = i*4;
			espDifusivo2D[i][j][0] = 0;
			if (bandera)
				continue;
			...
			//espDifusivo2D[i][j][s] = 0.5*(i+1)*(rand() % DIF);  
		//Inicializacion del espacio difusivo
		}
	}
...
int difusion_engine(void)
{
	...
	espDifusivo2D[10][12][0] += 15000; //Fuente de morfogen en [10,12]
		...
		for (i=0; i<LARGO; i++)
		{
			...
			{ 
				...
				else
				{
					...
					else if ( (espDifusivo2D[i][j][s] +=  0.184*(espDifusivo2D_old[i-1][j][s] + espDifusivo2D_old[i+1][j][s] + espDifusivo2D_old[i][j-1][s] + espDifusivo2D_old[i][j+1][s] - 4*espDifusivo2D_old[i][j][s])) < 0)
espDifusivo2D[i][j][s] = 0;
...
int genes_reguladores(int i)
{
	...
	for (l=0; l<MAXGEN_REG;l++)
	{
		...
		{
			X[k] = (*celulas[i]).Senales[k].afinidad[l][2] * (*celulas[i]).Senales[k].afinidad[l][0] * espDifusivo2D[(*celulas[i]).posicion.x][(*celulas[i]).posicion.y][k]/1200 - (*celulas[i]).Senales[k].afinidad[l][1];
			//LINEA LARGA DIVIDIDA
...
int genes_senales(int i)
{
	...
	for (l=0; l<MAXGEN_SEN;l++)
	{
		...
		if( ( (*celulas[i]).Senales[l].produccion = 1200*(1 + tanh(2*act_tot -5)) ) < 0)
			(*celulas[i]).Senales[l].produccion = 0;
		else
			espDifusivo2D[(*celulas[i]).posicion.x][(*celulas[i]).posicion.y][l] += (*celulas[i]).Senales[l].produccion;
	//LINEA LARGA DIVIDIDA
...
/////////////////////////////////////////////////////////////////////////////
	...
	for(i = 0; i<MAXGEN_REG; i++)
	{
		...
		(*cel).Reguladores[i].produccion = 0;
	...
	for(i = 0; i<MAXGEN_EST; i++)
	{
		...
		(*cel).Estructurales[i].produccion = 0;
	}
	...
	for (i=0; i<LARGO; i++)
	{
		...
		for(j=0; j<ANCHO; j++)
		{
			if (espDifusivo2D[i][j][MAXGEN_SEN])
				one.filledsquare((i * FACTOR),(j * FACTOR),(i * FACTOR + FACTOR),(j * FACTOR + FACTOR),65535,65535,65535);
		}
	}
...
int dibujar_difusion()
{
	static float contador2 = 0;
	int dec;
	int sing;
	int len, q, i, j,s;
	int rangocolor = (int) (65535 / 800);
	char nombre[30]="difusion";
	char numero[8]="0";
	char *numero_tmp;
	char *numero_fin;
	contador2++;
	numero_tmp = fcvt(contador2,0,&dec,&sing);
	strcat(numero, numero_tmp);
	numero_tmp = numero;
	numero_fin = strchr(numero,'\0');
	len = (int) (numero_fin - numero_tmp) / sizeof(char);
	...
	pngwriter one((ANCHO*FACTOR),(LARGO*FACTOR),65535,nombre);
	one.setcompressionlevel(9);
	...
	for (i=0; i<LARGO; i++)
	{
		...
		for(j=0; j<ANCHO; j++)
		{
			...
			one.filledsquare((i * FACTOR),(j * FACTOR),(i * FACTOR + FACTOR),(j * FACTOR + FACTOR),(int) (rangocolor*espDifusivo2D[i][j][0]),(int) (rangocolor*espDifusivo2D[i][j][0])/3, 0);
